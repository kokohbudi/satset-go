...
public class ProviderHttpConfig {
     ...
     * (no truncate, no redact — raw body masuk log).
     */
    private static ClientHttpResponse logSupplierCall(HttpRequest req, byte[] body,
        ...
        // full body, no truncate — logger com.satset.digiflazz punya file sendiri (logs/digiflazz/)
        log.info("Supplier ← {} {} → {} ({} ms, {} chars):\n{}", req.getMethod(), req.getURI(),
                resp.getStatusCode().value(), ms, respBody.length(), prettyJson(respBody));
        return resp;
    }
